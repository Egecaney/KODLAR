...
int main(void){
    ...
    while(1){
        ...
        uint8_t TempHigh[] = {24,25,25,25,25,25};
        uint8_t HumHigh[] = {20,25,29,34,44,56};
        sprintf(TX_Buffer,"Temp=,Humidity=",TempHigh[i],HumHigh[i]);
        i++;
        if(i==6){
            i=0;
        }
        Publish("g25","Temp=25.44");
...
void Get_Temperature_Humidity(void){
    ...
    if(FirstCounter_ForDHT>=40){
        ...
        //ltoa(TemperatureLow,TemperatureLow_Char,10);
        //sprintf(Temperature,"%s.%s",TemperatureHigh_Char,TemperatureLow_Char);
        //ltoa(HumidityHigh,HumidityHigh_Char,10);
        //ltoa(HumidityLow,HumidityLow_Char,10);
        //sprintf(Humidity,"%s.%s",HumidityHigh_Char,HumidityLow_Char);
