...
function Quantized = Midrise_Quantizer(Signal,Bit)
...
for i=1 : length(Signal)
    imp(i) = abs(floor(Signal(i)/Delta));
    if imp(i)< LevelQuantization
        Quantized(i) = (floor(Signal(i)/Delta) + 0.5)*Delta;
    elseif Signal(i)>0
        Quantized(i) = (LevelQuantization-0.5)*Delta;
    elseif Signal(i)<0
        Quantized(i) = (-LevelQuantization+0.5)*Delta;
    elseif Signal(i) == 0
        Quantized(i) = 0;
